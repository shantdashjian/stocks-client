...
async function fetchStockData() {
    ...
    try {
        const stockData = await Promise.all(tickersArr.map(async (ticker) => {
            const url = `https://polygon.stocks-shant.workers.dev/?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`
...
async function fetchReport(data) {
    ...
    try {
        ...
        const response = await fetch('https://stocks.stocks-shant.workers.dev/', options)
        const report = await response.json()
        if (!response.ok) {
            throw new Error(`Worker error: ${report.error}`)
        }
